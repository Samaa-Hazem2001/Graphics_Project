...
void World::deserialize(const nlohmann::json &data, Entity *parent) {
  ...
  for (const auto &entityData : data) {
    ...
    // Create an entity
    Entity *entity = add();
    // make its parent "parent"
    entity->parent = parent;
    // call its deserialize with "entityData".
    entity->deserialize(entityData);
    if (entityData.contains("children")) {
      // TODO: (Req 8) Recursively call this world's "deserialize" using the
      // children data
      //  and the current entity as the parent

      // call this world's deserialize(data = entityData["children"], parent =
      // entity) for all children
